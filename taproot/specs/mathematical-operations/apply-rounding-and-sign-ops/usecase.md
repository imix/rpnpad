...
1. User initiates a rounding or sign operation:
   - **`r›` chord (FLOOR/CEIL/TRUNC/ROUND/SIGN)**: User presses `r` (chord leader); hints pane switches to the `r›` submenu showing `f  ⌊x⌋`, `c  ⌈x⌉`, `t  trunc`, `r  RND↓`, `s  sgn`; user presses the second key.
2. Engine applies the operation:
   ...
   - **SIGN** (`rs`): replaces pos 1 with −1 (negative), 0 (zero), or +1 (positive)
...
- **Trigger:** User enters `r` chord leader (for any `r›` op including `rs`) while Insert mode has a non-empty buffer
- **Steps:**
...
- `r›` chord submenu (including `s  sgn`) appears in hints pane whenever stack depth ≥ 1

## Error Conditions
- **Stack underflow — unary op on empty stack**: User triggers `rf`/`rc`/`rt`/`rs` with an empty stack — error message on ErrorLine, stack unchanged
...
stateDiagram-v2
    ...
    ChordR --> Normal : s (SIGN) — stack ≥1 → result at pos 1
    ChordR --> Normal : f/c/t/s — stack empty → ErrorLine
...
- `../../discoverability/browse-hints-pane/usecase.md` — hints pane must expose `r›` submenu (including `s  sgn`) in Normal-mode view
...
- When the user presses `r` then `s`
...
- Then the `r›` chord leader appears in the Normal-mode view; in the `r›` submenu, `s  sgn` is visible
...
- `r` is the `r›` chord leader (Rotate was rebound to `R` (shift-r) to free it).
- `S` (shift-s) is `EnterStoreMode` (named registers) — SIGN uses `rs`, not `S`.

...
3. User presses `↑` to move cursor deeper into the stack (position 3,
   4…) or `↓` to move back toward the top (minimum position 2)
...
## Error Conditions
- **`↑` pressed with ≤1 item**: error on ErrorLine ("Roll requires ≥2
  items"), Browse mode not entered, stack unchanged
...
stateDiagram-v2
    ...
    Browse --> Browse : ↑ — cursor moves deeper (clamped at deepest)
...
**AC-1: Roll item from arbitrary depth to position 1**
- Given the stack has ≥3 items and Browse mode is active with cursor at position N (N ≥ 2)
...
**AC-5: Cursor clamped at deepest position**
- Given Browse mode is active with cursor at the deepest visible position
- When the user presses `↑`
- Then the cursor stays at the deepest position and Browse mode remains active
